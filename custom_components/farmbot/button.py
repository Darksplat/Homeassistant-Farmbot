...
class FarmbotRunSequenceButton(FarmbotEntity, ButtonEntity):
    ...
    def available(self) -> bool:
        return bool(self._manager.mqtt_connected and getattr(self._manager, "selected_sequence_id", None) is not None)

    async def async_press(self) -> None:
        sequence_id = getattr(self._manager, "selected_sequence_id", None)
        if sequence_id is not None:
            await self.hass.async_add_executor_job(self._manager.execute_sequence, sequence_id)
